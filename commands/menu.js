...
module.exports = {
  ...
  async execute(sock, m, args, { prefix, currentMode }) {
    ...
    const uptime = global.botStartTime ? formatUptime(Date.now() - global.botStartTime) : "N/A";
...
`;
    ...
    for (const cat of Object.keys(categories)) {
      const cmds = categories[cat].map(c => `${prefix}${c.toUpperCase()}`);
      menuText += `\n╭─❲ ☢️ ${cat} 𝚂𝙴𝙲𝚃𝙾𝚁 ❳──┈\n`;
